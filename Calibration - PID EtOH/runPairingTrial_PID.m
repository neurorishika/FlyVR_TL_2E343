function runPairingTrial_PID(Exp_Name,CSpWait, CSpLength, ledDelay, CSmWait, CSmLength, endWait, ledPulseWidth, ledIntensity, nTrials, trialDelay, sampleRate)
%% initialize camera
...
% PID Channel
Session.addAnalogInputChannel(NIdaq.dev,'ai0','Voltage');
...
time = (0.001:0.001:trialLength/sampleRate);
Session.DurationInSeconds = trialLength/sampleRate;
...
for trial = 1:nTrials
    ...
    lh = Session.addlistener('DataAvailable', @plotData);
    ...
    close(gcf);
    plot(time,data);
    csvwrite(strcat('pid_',num2str(10+trial),'.csv'),data)
    delete(lh);
    pause(trialDelay)
